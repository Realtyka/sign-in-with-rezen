...
//
// The server-side sample has the same function written against Node's crypto
// module — confidential-client/src/oidc.js. The two are deliberately
// identical in what they accept, what they reject, and what they say when
// they reject; ../../test-vectors/id-token.json is the shared vector both
// test suites run through their own copy. Change one and you must change the
// other, or that vector fails in the suite you didn't touch.
...
// the form body — never in the URL, never in a log line. A public client
// authenticates here exactly as it does at the token endpoint: client_id in
// the body, no secret, no Authorization header.
//
...
export async function revoke(discovery, { clientId, token, tokenTypeHint }) {
